...
@frappe.whitelist()
def purchase_overdue():
        purchase_overdue = frappe.db.sql("select supplier_name,grand_total,due_date from `tabPurchase Invoice` where status = 'Overdue' ", as_dict=True)
        msg = "Purchase Overdues:\n Name | Due Amt | Due Date \n"
        for purchase in purchase_overdue:
            single_overdue = str(purchase.supplier_name) + " | Rs " + str(purchase.grand_total) + "  |  " + str(purchase.due_date.strftime('%m/%d/%Y')) + "\n"
