...
class SalesInvoice(SellingController):
	...
	def on_submit(self):
                print("HIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII")
                msg = 'New Invoice Created:\n' + str(self.customer) + '\nAmount: ' + str(self.grand_total) + '\nBill Date:'+ str(self.posting_date) + '\nDue Date: ' + str(self.due_date)
                print(msg)
...
@frappe.whitelist()
def sales_overdue():
        sales_overdue = frappe.db.sql("select customer_name,rounded_total,due_date from `tabSales Invoice` where status = 'Overdue' ", as_dict=True)
        msg = "Sales Overdues:\n Name | Due Amt | Due Date \n"
        for sale in sales_overdue:
            single_overdue = str(sale.customer_name) + " | Rs. " + str(sale.rounded_total) + " | " + str(sale.due_date.strftime('%m/%d/%Y')) + "\n"
            msg+=str(single_overdue)
        p = plivo.RestAPI(auth_id, auth_token)
        params = {
	 'src': src,
	 'dst' : dst,
